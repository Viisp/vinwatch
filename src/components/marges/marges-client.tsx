...
import { matchOrderMargins, type OrderMarginMatch } from '@/lib/vinted-calculations';
import { Scale, ArrowRight, PackageCheck } from 'lucide-react';
...
export function MargesClient() {
  const [matches, setMatches] = useState<OrderMarginMatch[]>([]);
  ...
  useEffect(() => {
    getOrders().then((data) => {
      setMatches(matchOrderMargins(data));
      setLoading(false);
    });
  }, []);
