...
export function parseVintedProfile(html: string): VintedProfile | null {
  ...
  // Prefer a small thumbnail (purpose-generated by Vinted for exactly this
  // kind of avatar use) over the full-size photo, which looks soft when a
  // large portrait image gets crammed into a 26px circle by the browser.
  const thumbnailMatch = slice.match(/\\"thumbnails\\":\[\{\\"type\\":\\"[^"\\]+\\",\\"url\\":\\"([^"\\]+)\\"/);
  const photoUrlMatch = slice.match(/\\"photo\\":\{\\"id\\":\d+,[\s\S]*?\\"url\\":\\"([^"\\]+)\\"/);

  return {
    login: loginMatch[1],
    profileUrl: profileUrlMatch ? profileUrlMatch[1] : null,
    photoUrl: thumbnailMatch ? thumbnailMatch[1] : photoUrlMatch ? photoUrlMatch[1] : null,
  };
}
